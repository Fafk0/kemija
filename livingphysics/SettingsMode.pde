...
void drawSettingsMode()
{
  ...
  String challenge_text = "Iziu "+str(iChallenge+1)+": \""+challenges[iChallenge].title+"\"\n\n" +
  ...
  if(asking_can_delete_reaction)
    showYesNoQuestion("Zbrisi reakcijo?",GLOBAL_ALPHA);
  if(asking_can_delete_all_reactions)
    showYesNoQuestion("Zbrisi vse reakcije?",GLOBAL_ALPHA);
  if(asking_can_reload_challenge)
    showYesNoQuestion("Restarti iziv?",GLOBAL_ALPHA);
  if(showing_message)
